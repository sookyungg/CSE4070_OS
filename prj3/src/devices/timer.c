...
timer_sleep (int64_t ticks) 
{
  ...
  list_push_back(&sleep_queue, &thread_current()->elem);
  thread_block();
...
timer_interrupt (struct intr_frame *args UNUSED)
{
  ...
  struct list_elem *element = list_begin(&sleep_queue);
  struct thread *current = list_entry(element, struct thread, elem);
  struct list_elem *end = list_end(&sleep_queue);
  ...
  if(element != end){
    ...
    current = list_entry(element, struct thread, elem);
	}
  else break;
  }
