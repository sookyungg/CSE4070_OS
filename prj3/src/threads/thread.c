...
void thread_set_nice (int new_nice) 
{
  ...
  new_current = current->priority;

  //calc
  new_current = (PRI_MAX*FRACTION) - (current->recent_cpu/4) - (new_nice*2);
  
  if(new_current > PRI_MAX) new_current = PRI_MAX;
  if(new_current < PRI_MIN) new_current = PRI_MIN;
}
  ...
  while(1){
    if(element!=list_end(&all_list)){
      if(thread != idle_thread)
        temp = ((int64_t)((int64_t)2*load_avg)*FRACTION / (2*load_avg+FRACTION)) * thread->recent_cpu/FRACTION + thread->nice*FRACTION;
        thread->recent_cpu = temp;
